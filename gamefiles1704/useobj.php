<?
         $page.="<p class='d'><b>".$location[obj_list][$_GET['var']][name]." <a href='./?view=obj&amp;var=$_GET[var]'><img src='/img/icon/i.PNG'/></a></b></p>";
         if ($location[obj_list][$_GET['var']][type]=="safe") {           if ( $location[obj_list][$_GET['var']][status][is_open]=="lock") {
           	     $page.="<br/><img src='/img/icon/cancel.PNG'/> Невозможно открыть! Замок заклинило!<br/>";
           }
           elseif ( $location[obj_list][$_GET['var']][status][is_open]!="open") {
         	  if (($player[skill][hack][level]- $location[obj_list][$_GET['var']][status][hard]+50)>rand(1,100)) {                  $location[obj_list][$_GET['var']][status][is_open]="open";
                  ...
                  $page.="<br/><img src='/img/icon/ok.PNG'/> Вы взломали замок! И получили $temp[exp] опыта!<br/>";         	  }
         	  else {$page.="<br/><img src='/img/icon/cancel.PNG'/> Не  удалось взломать замок!";
...
if((time()-$status[digtime])<30){$page.='<br><b id="txt">Вы устали, передышка!</b> <b id="t">'.(30+$status[digtime]-time()).'</b><script>
var seconds='.(30+$status[digtime]-time()).';
...
textNode.nodeValue="";
textNode=document.getElementById("txt").firstChild;
textNode.nodeValue="Передышка окончена!";
var ct=document.getElementById("txt");
ct.style["color"]	="#3c3";
}
}
display();
...
$page.=" <br/><img src='/img/icon/arrow.PNG'/> <a href='./'>Уйти</a><br/>";
